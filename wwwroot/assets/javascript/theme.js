    ...
    value: function watchMQ() {
      ...
      $(window).on('resize', function () {
        // force close aside on toggle screen up
          if (_this6.isToggleScreenUp() && $('.app-aside').hasClass('has-open') &&
...
var HRMS = function () {
    var HRMS = new Theme(); // toggle skin thought button

  $('[data-toggle="skin"]').on('click', function (e) {
    e.preventDefault();
      var skin = HRMS.skin === 'dark' ? 'default' : 'dark';
      HRMS.setSkin(skin); // we need to refresh our page after change the skin

    location.reload();
  }).each(function () {
      var isDarkSkin = HRMS.skin === 'dark';
